...
selected_config=$(printf "%s\n" "${WAYBAR_CONFIGS[@]}" | yad --list --column="Configuration" --separator="" --no-headers --title="Choose Configuration" --text="" --center --button="   Close me if you dare ..." --width=600 --height=306 --fixed --image="$logo_path")
...
pgrep -x awww-daemon >/dev/null || (awww-daemon & disown)

# change wallpaper using swww
awww img "${DIR}/${RANDOMPICS}" --transition-step 20 --transition-fps=20
